...
def reporte_ventas_diarias(dias: int = 7, db: Session = Depends(get_db)):
    ...
    return [VentaDiaria(fecha=str(r.fecha), total_ventas=r.total or 0.0) for r in resultados]


@router.get("/productos-mas-vendidos", response_model=List[ProductoMasVendido])
def reporte_productos_mas_vendidos(limit: int = 10, db: Session = Depends(get_db)):
